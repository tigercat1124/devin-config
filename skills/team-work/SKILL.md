...
See `AGENTS.md` → `### Role profiles` for the subagent role definitions.
...
See `AGENTS.md` → `### Subagent hard rules` for the underlying file-search limits (no retries, no all-matching patterns, max three searches, stop on missing files).
...
See `AGENTS.md` → `### Team orchestration rules` for the no-auto-push policy.
